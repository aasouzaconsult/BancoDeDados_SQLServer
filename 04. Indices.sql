...
BEGIN
	...
	IF @Included IS NULL
		print('/* ' + CONVERT(VARCHAR(14), @Advantage) + '% */ CREATE INDEX I_EMPRESA_' + convert(varchar(8), @Handler)) + ' ON ' + @TableStatement + ' (' + @Columns + ')'
	ELSE
		print('/* ' + CONVERT(VARCHAR(14), @Advantage) + '% */ CREATE INDEX I_EMPRESA_' + convert(varchar(8), @Handler)) + ' ON ' + @TableStatement + ' (' + @Columns + ') INCLUDE (' + @Included + ')'
...
WHERE	DB_NAME(database_id) = 'banco' -- <- Informe o nome do banco de dados
and     I.Name like '%nome%' --<- Informe um nome para consultar (string que precisa pesquisar)
...
WHERE Parent_Object_ID = OBJECT_ID('CARGOS') -- <- Nome da Tabela